...
- `douban-report-full.png`：报告全页长图（自动生成）
- `douban-report-charts.png`：图表部分分享图（自动生成）
- `douban-report-insights.png`：AI 洞察部分分享图（自动生成）
...
7. **生成 AI 洞察**（核心增值步骤）：
   ...
   - 提示用户刷新报告页面查看
8. 脚本会自动截取报告长图（`douban-report-full.png`）并按内容拆分为 2 张分享图（图表 + AI 洞察），告知用户图片位置
